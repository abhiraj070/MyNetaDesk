...
```mermaid
flowchart TD
    A[User opens myNeta] --> B{Grant location?}
...
- **`app/main.py`** creates the FastAPI app (`title="myNeta"`), configures CORS from the `CORS_ORIGINS` env var, and registers routes via a side-effect import of `app/api/user.py`. It owns a **lifespan** that starts `app/tasks/daily_reset.py`.
...
- **`app/config/settings.py`** loads `app/.env` via pydantic-settings (`DB_URL`, `BEARER_TOKEN_X`).
...
> **Naming note:** `slap_count` / `rose_count` are the **voting mechanic**, not the brand — they are intentionally left as-is. The app name ("myNeta") lives only in UI copy, metadata, and pipeline User-Agent strings; it does not appear in the schema.

---

## API reference

Base URL: `http://localhost:8000` in development (frontend reads `NEXT_PUBLIC_API_BASE_URL`).
